...
def process_frame(frame):
   ...
   # determining_point=frame_center_x
    ...
    for cnt in obstacle_contour:
        ...
        cv2.circle(edges, (int(cx), int(cy)), int(np.sqrt(area/np.pi)), (255, 0, 0), 2)
        break
    trash_exists, trash_debug, trash_mask = detect_trash(frame)
#    cv2.imshow("img", edges)
#    cv2.imshow("trash_mask", trash_mask)
 #   cv2.waitKey(0)
